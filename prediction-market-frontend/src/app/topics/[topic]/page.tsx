...
export default function TopicPage() {
  ...
  const markets = getMarketsByCategory(topic);
  
  const [filter, setFilter] = useState("newest");
  
  const topicTitles: Record<string, string> = {
    crypto: t("topics.crypto"),
    sports: t("topics.sports"),
    politics: t("topics.politics"),
    economy: t("topics.economy"),
  ...
  const filteredMarkets = [...markets]
    .filter(market => {
      if (filter === "open") {
        return market.status === "active" || !market.status;
      }
      return true;
    })
    .sort((a, b) => {
  ...
  const topicTitle = topicTitles[topic] || topic.charAt(0).toUpperCase() + topic.slice(1);

  return (
    <div className="w-full min-h-screen bg-[#111212] px-6 py-6">
      <div className="max-w-[1400px] mx-auto">
        <h1 className="text-white text-3xl font-bold mb-6">{topicTitle}</h1>
                  ...
                  {market.options.map((option, idx) => (
                    <div key={idx} className="flex items-center justify-between">
                      <button className="flex-1 bg-[#2A2A2A] hover:bg-[#F3BA2F] hover:text-black text-white px-3 py-1.5 rounded-lg text-xs font-medium transition-colors mr-2">
                        {getTranslatedOptionText(option.text, t, i18n)}
                      </button>
        ...
        {filteredMarkets.length === 0 && (
          <div className="text-center text-[#838587] py-20">
            <p className="text-lg">{t("common.noMarketsFound")}</p>
          </div>
        )}
      </div>
    </div>
